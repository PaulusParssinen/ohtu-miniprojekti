...
class TestReadingTipService(unittest.TestCase):
    ...
    def test_search_by_title_returns_false_if_not_found(self):
        self.assertFalse(self._reading_tip_service.search_by_title("Unknown"))
    
    def test_search_by_title_returns_true_if_found(self):
        self.assertTrue(self._reading_tip_service.search_by_title("Kirja 1"))

    def test_get_reading_tip_with_existing_reading_tip(self):
        reading_tip = self._reading_tip_service.get_by_id(1)
        self.assertEqual(1, reading_tip.id)

    def test_get_reading_tip_with_nonexisting_reading_tip(self):
        self.assertEqual(None, self._reading_tip_service.get_by_id(100))

    def test_update_reading_tip_by_tip_id(self):
        self._reading_tip_service.update_by_id(1, 'Muutettu Kirja 1')
        reading_tip = self._reading_tip_service.get_by_id(1)
